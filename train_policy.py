...
logging.basicConfig(format='%(asctime)s¥t%(levelname)s¥t%(message)s', datefmt='%Y/%m/%d %H:%M:%S', filename=args.log, level=logging.DEBUG)
...
# 保存済のpickleファイルがある場合、pickleファイルを読み込む
#train data
...
if os.path.exists(train_pickle_filename):
    ...
    positions_train = read_kifu(args.kifulist_test)
...
# 保存済のpickleがない場合、pickleファイルを保存する
...
def mini_batch(positions, i, batchsize):
    ...
    return Variable(cuda.to_gpu(np.array(mini_batch_data, dtype=np.float32))), Variable(
        cuda.to_gpu(np.array(mini_batch_move, dtype=np.int32)))
    ...
    for e in range(args.epoch):
        ...
        for i in range(0, len(positions_train_shuffled) - args.batchsize, args.batchsize):
            ...
            if optimizer.t % args.eval_interval == 0:
                ...
                sum_loss = 0
        # validate test data
        logging.info('variable test data')
        itr_test = 0
        sum_test_accuracy = 0
